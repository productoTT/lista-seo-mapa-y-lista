...
export function FiltersModal({
  ...
  return createPortal(
    <>
      {/* Overlay */}
      <div
        style={{ position: 'fixed', inset: 0, background: 'rgba(34,17,96,0.4)', zIndex: 1300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px 16px' }}
        onClick={onClose}
      >
        {/* Modal */}
        <div
          style={{
            ...
            zIndex: 1301,
            boxShadow: '0 8px 48px rgba(34,17,96,0.18)',
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: `1px solid ${DIVIDER}`, flexShrink: 0 }}>
            <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: FG1 }}>Más filtros</h3>
          ...
          {/* Footer */}
          <div style={{ padding: '12px 24px', borderTop: `1px solid ${DIVIDER}`, display: 'flex', gap: 10, flexShrink: 0 }}>
            <button
              onClick={onClear}
              style={{ padding: '10px 20px', border: `1px solid ${DIVIDER}`, borderRadius: 4, background: '#fff', color: '#666', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
            >
              Limpiar filtros
            </button>
            <button
